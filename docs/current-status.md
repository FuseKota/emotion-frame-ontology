...
| **10** | **集約関数比較** | **完了** | `output/experiment/aggregation_study.json` |

Step 7–9 および Fig 7–9 は 2026-02-15 に新規実装・実行完了。
Step 10 および Fig 10 は 2026-02-16 に新規実装・実行完了。
...
### 3.4 集約関数比較 — Ablation Study (Step 10)

現行の min() 集約関数を 6 種の代替関数と体系的に比較。
主指標: Spearman ρ (dyadScore vs SemEval intensity)。閾値なし（連続値）。
全 28 検定に Holm 補正適用。Bootstrap CI (n=2,000)。
...
| Dyad | 最良手法 | ρ (最良) | ρ (min) | Δρ |
|------|---------|---------|---------|-----|
| Love | OWA (0.3/0.7) | +0.504 | +0.349 | **+0.155** |
| Disapproval | Power Mean (p=-2) | +0.639 | +0.636 | +0.003 |
| Optimism | OWA (0.3/0.7) | +0.519 | +0.165 | **+0.355** |
| Contempt | Product | +0.573 | +0.509 | **+0.064** |

#### 主要な発見

- **OWA (0.3/0.7) が全体平均で最も高い ρ** (Mean = +0.544 vs min の +0.415、+0.129 改善)
- **Optimism で最大の改善**: min の ρ=+0.165 → OWA の ρ=+0.519 (Δρ=+0.355)
  - 理由: Optimism = Anticipation + Joy で、Joy が SemEval intensity と直接対応するため、弱い方の Anticipation だけに引きずられる min() では情報損失が大きい
- **Disapproval は min() が堅調**: 代替関数による改善はわずか (Δρ < 0.003)
- **Łukasiewicz は不適切**: ゼロ率が 75–99% と高く、Spearman ρ が大幅低下
- **Product と Geometric Mean は同一の Spearman ρ**: 単調変換のため順位相関が一致
- **偏相関は全関数で小さい**: 成分の決定的関数である限り、増分価値は本質的に制約される

#### 論文での活用

- **Ablation Study** (Discussion セクション) として、min() 選択の実験的根拠を提示
- OWA が全体的に優位だが、理論的根拠 (Gödel t-norm としての min()) との
  トレードオフを議論材料として活用
- Łukasiewicz の失敗は「なぜ暗黙閾値が不適切か」の説得力ある実証

### 3.5 既存評価結果 (Steps 4–5) まとめ
...
| **`output/experiment/aggregation_study.json`** | **集約関数比較 (step10)** |
...
| **10** | **`aggregation_comparison.png`** | **集約関数比較ヒートマップ + 散布図** | **Fig 5 (Ablation)** |
...
| Table 5 | Ablation | 集約関数 × Dyad の Spearman ρ 比較行列 | `aggregation_study.json` | **データ生成済み** |
| Fig 5 | Ablation | 集約関数ヒートマップ + 散布図比較 | Fig 10 | **生成済み** |
...
| min() の集約関数選択が恣意的 | 中 | **解決済み**: 7 集約関数の Ablation Study (Step 10) で体系的に比較 | Table 5 / Fig 5 で報告 |
...
# Step 10: 集約関数比較 (Step 7 のキャッシュが必要、数秒で完了)
python -m experiment.step10_aggregation_study

# Fig 7–10 のみ生成
python -m experiment.step6_visualize --only 7 8 9 10
```
...
# 新規ステップをスキップ
python -m experiment.run_pipeline \
  --skip-step7 --skip-step8 --skip-step9 --skip-step10
```
